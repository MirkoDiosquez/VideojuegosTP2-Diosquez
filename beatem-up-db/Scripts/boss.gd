...
func _follow() -> void:
	...
	velocity = Vector2.ZERO if dist < ATTACK_DISTANCE else direction * SPEED
...
func _on_animation_finished() -> void:
	if anim.animation == "boss_attack1":
		is_attacking = false
		if player != null:
			player.take_damage(ATTACK_DAMAGE)
	elif anim.animation == "boss_getHited":
		is_hit = false
	elif anim.animation == "boss_death":
		get_tree().change_scene_to_file("res://Escenas/you_win.tscn")
